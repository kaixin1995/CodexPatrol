...
- **保存后同步 CPA 优先级**：点击保存后会同步更新 CPA 的 `priority` 字段，仅影响 CPA 的选路顺序；本地越小越优先，写入 CPA 时会自动换算为“数值越大越优先”
...
保存配置后，会立即同步 CPA 的 `priority` 字段，但不会立刻切换账号状态；需要手动触发一次巡检，或等待下一次自动巡检周期执行后，优先级路由调度才会真正生效。若 CPA 同步失败，本地配置仍会保留，并在页面给出告警提示。
