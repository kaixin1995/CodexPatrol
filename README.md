...
- **动态窗口适配**：按账号实际返回的额度窗口动态判断，不再硬编码“免费号只看周额度 / 收费号看周额度+5小时额度”
- **额度超限检测**：任一实际返回的额度窗口达到阈值（默认 95%）时，建议禁用该账号
- **恢复检测**：已禁用账号只有在所有实际返回的额度窗口都恢复到阈值以下时，才视为可恢复；优先级路由关闭时建议立即启用，开启时转入待命，等待优先级调度
- **窗口标签修正**：仅在上游缺少 `limit_window_seconds` 时才按顺序推断 5 小时 / 周窗口，避免把长周期窗口误标为“5 小时限额”
...
- **动态额度窗口展示**：按账号实际返回的额度窗口展示使用率与重置时间，不再假设免费号固定只有周额度、收费号固定只有周额度和 5 小时额度
- **动态窗口适配**：按账号实际返回的额度窗口动态判断，不再硬编码“免费号只看周额度 / 收费号看周额度+5小时额度”
- **额度超限检测**：任一实际返回的额度窗口达到阈值（默认 95%）时，建议禁用该账号
- **恢复检测**：已禁用账号只有在所有实际返回的额度窗口都恢复到阈值以下时，才视为可恢复；优先级路由关闭时建议立即启用，开启时转入待命，等待优先级调度
- **窗口标签修正**：仅在上游缺少 `limit_window_seconds` 时才按顺序推断 5 小时 / 周窗口，避免把长周期窗口误标为“5 小时限额”
...
|轮询间隔|10 分钟（最小 1 分钟）|自动巡检间隔|
|随机抖动|0~3 分钟|避免固定整点齐发；最小值和最大值都允许为 0|
|探测并发|3|同时探测的账号数|
|操作并发|4|同时执行动作的并发数|
|请求超时|15 秒|单次请求超时|
|重试次数|0|请求失败重试|
